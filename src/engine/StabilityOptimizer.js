...
class PriorityUpdateQueue {
  ...
  remove(pieceId) {
    const priority = this.pieceToPriority.get(pieceId);
    if (priority !== undefined) {
      this.queues.get(priority).delete(pieceId);
      this.pieceToPriority.delete(pieceId);
    }
  }
  ...
  getCount(priority = null) {
    ...
    return this.pieceToPriority.size;
  }
...
export class StabilityOptimizer {
   ...
   * Adapted for BuildingValidator which may not have calculateStability
   */
  calculateAndCache(piece) {
    // For turrets, stability is simple - grounded = stable
    let stability = 1.0;
    
    if (this.validator.calculateStability) {
      stability = this.validator.calculateStability(piece);
    } else {
      // Simple stability check: grounded pieces are stable
      const pieceData = this.validator.graph?.pieces?.get(piece.id);
      if (pieceData) {
        stability = pieceData.isGrounded ? 1.0 : 0.0;
      }
    }
    ...
    // Track dependencies if validator has graph
    if (this.validator.graph?.getSupports) {
  ...
  processBatch(maxCount, maxPriority) {
    ...
    // Get pieces from validator
    const pieces = batch
      .map(({ pieceId }) => {
        if (this.validator.graph?.pieces) {
          return this.validator.graph.pieces.get(pieceId);
        }
        return null;
      })
      .filter(p => p)
  ...
  precomputeRegion(pieces) {
    // Sort bottom to top
    const sorted = [...pieces].sort((a, b) => (a.position?.y ?? 0) - (b.position?.y ?? 0));
